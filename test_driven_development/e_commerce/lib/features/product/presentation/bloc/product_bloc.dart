...
class ProductBloc extends Bloc<ProductEvent, ProductState> {
  ...
  ProductBloc({required this.getOneProductUsecase, 
    ...
    on<GetAllProductsEvent>((event, emit) async {
      ...
      on<AddProductEvent>((event, emit) async {
       emit(
        ...
        final responce = await addProductUsecase(AddParams(product:event.product,imageFile: event.imageFile));
        responce.fold(
          (failure) {
      ...
      on<UpdateProductEvent>((event, emit) async {
        ...
        final responce =
            await updateProductUsecase(UpdateParams(product:event.product, id:event.id,imageFile: event.imageFile));
        responce.fold(
          (failure) {
            emit(ProductActionFailer(message: failure.toString()));
          },
